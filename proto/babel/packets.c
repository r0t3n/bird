...
#define PLEN_BYTES(plen) ((plen + 7U)/8)
                  ...
                  struct babel_parse_state *state)
{
  ...
  int len = PLEN_BYTES(tlv->plen) - (int) tlv->omitted;
  u8 buf[16] = {};
                   ...
                   struct babel_write_state *state, uint max_len)
{
  ...
  uint len = sizeof(struct babel_tlv_update) + PLEN_BYTES(msg->plen);
                         ...
                         struct babel_parse_state *state UNUSED)
{
  ...
  case BABEL_AE_IP6:
    ...
    if (TLV_OPT_LENGTH(tlv) < PLEN_BYTES(tlv->plen))
      return PARSE_ERROR;
			  ...
			  struct babel_write_state *state UNUSED, uint max_len)
{
  ...
  uint len = sizeof(struct babel_tlv_route_request) + PLEN_BYTES(msg->plen);
			  ...
			  struct babel_write_state *state UNUSED, uint max_len)
{
  ...
  uint len = sizeof(struct babel_tlv_seqno_request) + PLEN_BYTES(msg->plen);
